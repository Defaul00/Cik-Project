...
*/
    ...
    function onMouseMove(e) {
        gsapWithCSS.to(cursor, .0125, {
            x: e.pageX - 5,
            y: e.pageY - 5
        });
    }
    let blocks = document.querySelectorAll(".fly-item__hourse");
    let duration = .4;
    let repeatDelay = .075 * (blocks.length - 1);
    gsapWithCSS.from(".fly-item__hourse", 5, {
        scale: 0,
        top: "50%",
        left: "50%",
    ...
    blocks.forEach((block => {
        block.addEventListener("click", (event => {
            ...
            if (previousGif) previousGif.remove();
    ...
    }));
    {
        ...
        class Image {
            ...
            isActive() {
                return gsapWithCSS.isTweening(this.DOM.el) || this.DOM.el.style.opacity != 0;
            }
        }
        class ImageTrail {
            constructor() {
            ...
            showNextImage() {
                ...
                gsapWithCSS.killTweensOf(img.DOM.el);
                ...
                gsapWithCSS.timeline().set(img.DOM.el, {
                    startAt: {
                        opacity: 0,
                        scale: 1,
        ...
        const preloadImages = () => new Promise(((resolve, reject) => {
            imagesLoaded(document.querySelectorAll(".hero .content__img"), resolve);
        }));
        preloadImages().then((() => {
            new ImageTrail;
        }));
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        const el = document.querySelector(".counter__value");
        if (el) {
            const state = { val: 0 };
            gsapWithCSS.to(state, {
                ...
                ease: "power2.inOut",
                onUpdate() {
                    el.textContent = `${Math.floor(state.val)}%`;
                }
            });
        }
    ...
    gsapWithCSS.from(".loader-1", {
        width: 0,
        duration: 6,
        ease: "power2.inOut"
    });
    gsapWithCSS.from(".loader-2", {
        width: 0,
        delay: 1.9,
        duration: 2,
        ease: "power2.inOut"
    ...
    gsapWithCSS.to(".loading-screen", {
        opacity: 0,
        duration: .5,
        delay: 7.5,
        ease: "power1.inOut"
    });
    gsapWithCSS.from("h1", 2, {
        delay: 7.3,
        y: 40,
        opacity: 0,
        ease: "power4.inOut",
    ...
    gsapWithCSS.fromTo(".action-header__button", {
        y: -40,
        opacity: 0
    }, {
        y: 0,
    ...
    function animateDecorCircle(circle, sectionRect) {
        const startRotation = getRandomNumber(0, 360);
        const endRotation = startRotation + 360;
        gsapWithCSS.set(circle, {
            x: getRandomNumber(0, sectionRect.width),
            y: getRandomNumber(0, sectionRect.height),
            rotation: startRotation
        });
        ...
        tl.to(circle, {
            ...
            y: () => getRandomNumber(0, sectionRect.height),
            rotation: endRotation,
            duration: () => getRandomNumber(100, 200)
        });
    }
    ...
    Array.from(decorImages).forEach((img => {
    ...
    gsapWithCSS.to(".animatable-image", {
        scrollTrigger: {
